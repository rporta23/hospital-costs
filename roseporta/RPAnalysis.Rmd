...
```{r}
confusion_matrix(knn_pred, df_test$costs_bin)
```
...
quant_predictors_train <- train_data |> 
  select(-c(control_bin_Governmental, control_bin_Proprietary, 
            provider_bin_Specialized, fold, costs_bin)) |> 
  scale()

qual_predictors_train <- train_data |> 
  select(control_bin_Governmental, control_bin_Proprietary,
         provider_bin_Specialized, costs_bin, fold) 

hpc_scaled_train <- cbind(quant_predictors_train, qual_predictors_train) 

quant_predictors_test <- test_data |> 
  select(-c(control_bin_Governmental, control_bin_Proprietary, provider_bin_Specialized, costs_bin)) |> 
  scale()

qual_predictors_test <- test_data |> 
  select(control_bin_Governmental, control_bin_Proprietary, provider_bin_Specialized, costs_bin) 

hpc_scaled_test <- cbind(quant_predictors_test, qual_predictors_test)

x_train <- model.matrix(costs_bin ~ . - fold, data = hpc_scaled_train)[ , -1]
y_train <- hpc_scaled_train$costs_bin

x_test <- model.matrix(costs_bin ~ ., data = hpc_scaled_test)[ , -1]
y_test <- hpc_scaled_test$costs_bin
```
...
test_error_ridge_bin <- mean( hpc_scaled_test$costs_bin !=  pred_class )
...
cv_error_ridge_bin <- kfold_cv_ridge_bin(hpc_scaled_train, lambda_min, alpha = 0)
cv_error_ridge_bin
```
...
test_error_lasso_bin <- mean( hpc_scaled_test$costs_bin !=  pred_class )
...
```{r}
cv_error_lasso_bin <- kfold_cv_ridge_bin(hpc_scaled_train, lambda_min, alpha = 1)
cv_error_lasso_bin
```
