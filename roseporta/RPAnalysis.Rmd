...
quant_scaled <- hpc_dummies |> 
  select(-c(control_bin_Governmental, control_bin_Proprietary, 
            provider_bin_Specialized, rural, costs_bin)) |> 
  scale()
...
df_train <- hpc_scaled |>  filter(set == "Train") |> select(-set)
df_test <- hpc_scaled |> filter(set == "Test") |> select(-set)
```
...
bag_pred_avg <- map_dbl(1:nrow(test_data), 
...
compute_MSEP_bag <- function(train_data, formula, i){
  ...
  bag_pred_avg <- map_dbl(1:nrow(current_test_fold), 
                                 ~mean(bag_pred$individual[.x,])
                                 )
...
compute_error_rf <- function(train_data, formula, i){
  ...
  rf_pred_avg <- map_dbl(1:nrow(current_test_fold), 
...
rf_pred_avg <- map_dbl(1:nrow(test_data), 
                                 ~mean(rf_pred$individual[.x,])
                                 )
...
cv_error_rate_optimal_lambda <- 0.08466906
...
find_mode <- function(x) {
  ...
  mode <- u[tab == max(tab)]
  return(mode[1])  # taking the first element arbitrarily in the rare case there's a tie.
}

test_predictions_majority <- 
  matrix(bag_pred$individual[ , 1], ncol = 1)
for (i in 2:n_trees) {
  test_predictions_majority <-
    cbind(
      test_predictions_majority,
      apply(bag_pred$individual[ , 1:i], 1, find_mode)
    )
}

test_error_for_all_trees <- colMeans(test_predictions_majority != test_data$costs_bin)


test_error_bag_cl <- mean(test_error_for_all_trees)
test_error_bag_cl
```
...
compute_error_bag_cl <- function(train_data, formula, i){
  ...
  test_error_for_all_trees <- 
    colMeans(test_predictions_majority != current_test_fold$costs_bin)
  
  avg_error <-  mean(test_error_for_all_trees)
...
compute_error_rf_cl <- function(train_data, formula, i){
  ...
  test_predictions_majority <- 
    matrix(rf_pred$individual[ , 1], ncol = 1)
  for (i in 2:n_trees) {
    test_predictions_majority <-
      cbind(
        test_predictions_majority,
        apply(rf_pred$individual[ , 1:i], 1, find_mode)
      )
  }
...
test_error_rf_cl <- mean(test_error_for_all_trees)
test_error_rf_cl
...
test_error_boost <- mean( (boost_pred != test_data$costs_bin) )
...
```{r}
cm_summary <- bind_rows(cm_logistic, cm_logistic2, cm_fwd_bin, 
...
```{r}
method <- c("Logistic Regression (Main Effects)", "Logistic Regression (Transformations)",
            "Forward Stepwise", "Backward Stepwise", "Ridge", "Lasso", "KNN",
            "LDA", "QDA", "Naive Bayes- Gaussian", "Naive Bayes- KDE")

cv_error <- c(cv_error_logistic, cv_error_logistic2, cv_error_fwd_bin, 
              cv_error_bwd_bin, cv_error_ridge_bin, cv_error_lasso_bin,
              cv_error_knn, cv_error_lda, cv_error_qda, 
              cv_error_nb, cv_error_nb_kde)

test_error <- c(test_error_logistic, test_error_logistic2, test_error_fwd_bin, 
