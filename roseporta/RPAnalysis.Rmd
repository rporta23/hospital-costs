...
cv_test_error <- kfold_cv_lm(train_data, formula)
...
cv_error <- cv_error_vec[optimal_size]
cv_error
```
...
```{r}
test_error <- compute_MSEP_test(best_formula, train_data, test_data)
...
error_ridge <- mean( (hpc_scaled_test$total_costs - pred_ridge) ^ 2 )
...
error_lasso <- mean( (hpc_scaled_test$total_costs - pred_lasso) ^ 2 )
...
test_error = mean( (hpc_scaled_test$total_costs - pred_test)^2 )
...
kfold_cv_logistic(train_data, formula)
```
