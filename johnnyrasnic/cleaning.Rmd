...
for (i in 1:10) {
  ...
  for (j in 1:ncol(train.X)-1) {
    ...
    for (k in 1:ncol(train.X)-1) {
      ...
      if (cond == TRUE) {selected_vars = append(selected_vars, colnames(summ)[-1][k])}
    }
    train.X = train.X |> select(all_of(selected_vars))
    lin_model = lm(total_costs ~ ., data=train.X)
  }
}
summary(best_subset)
...
```{r}
library(leaps)
best_subset = regsubsets(total_costs ~ ., data = train.X, method="forward", nvmax = ncol(train.X)-1)
summary(best_subset)
```
...
```{r}
library(leaps)
best_subset = regsubsets(total_costs ~ ., data = train.X, method="backward", nvmax = ncol(train.X)-1)
summary(best_subset)
```
