...
coefficients_df = hpc_dummies |> select(-c(total_costs, costs_bin, set, fold)) |> mutate(across(everything(), as.numeric))
MSE_df = coefficients_df[1:10,]

for (i in 1:10) {
  df_train_i = hpc_dummies |> filter(fold == i) |> select(-c(fold, set))
...
```{r}
train.X = df_train |> select(-c("costs_bin")) |> mutate(beds2 = number_of_beds^2)
test.X = df_test |> select(-c("costs_bin")) |> mutate(beds2 = number_of_beds^2)
...
```{r}
library(leaps)
best_subset = regsubsets(total_costs ~ ., data = hpc_dummies[,-c("costs_bin", "set")])
summary(best_subset)
```
...
```{r}
library(leaps)
best_subset = regsubsets(total_costs ~ ., data = hpc_dummies[,-c("costs_bin", "set", "fold")], method="forward")
summary(best_subset)
```
...
```{r}
library(leaps)
best_subset = regsubsets(total_costs ~ ., data = hpc_dummies[,-c("costs_bin", "set", "fold")], method="backward")
summary(best_subset)
```
...
```{r}
library(glmnet)
x = scale(model.matrix(total_costs ~ ., data = hpc_dummies[,-c("costs_bin", "set", "fold")])[ , -1])
...
lambda_grid = 10 ^ seq(10, -2, length = 100)
lasso_reg = cv.glmnet(x, hpc_dummies$total_costs, alpha = 1, type.measure = "mse", lambda = lambda_grid, nfolds=10)
```
...
for (i in 1:10) {
  train.X = hpc_dummies |> filter(fold != i) |> select(-c(set,fold,costs_bin))
  test.X = hpc_dummies |> filter(fold == i) |> select(-c(set,fold,costs_bin))
...
for (i in 1:10) {
  train.X = hpc_dummies |> filter(fold != i) |> select(-c("total_costs", "set", "fold"))
  test.X = hpc_dummies |> filter(fold == i) |> select(-c("total_costs", "set", "fold"))
