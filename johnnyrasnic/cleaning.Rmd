...
for (i in 1:10) {
  train.X = df_train |> filter(fold != i) |> select(-costs_bin)
  test.X = df_test |> filter(fold == i) |> select(-costs_bin)
...
pred = predict(bag.costs_bin, newdata=test.X)
...
for (i in 1:10) {
  train.X = df_train |> filter(fold != i) |> select(-total_costs)
  test.X = df_test |> filter(fold == i) |> select(-total_costs)
