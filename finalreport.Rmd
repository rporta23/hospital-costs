...
#### Results
Using cross validation, we obtained $\lambda = 0.01$ as the optimal tuning parameter. We obtained the same MSEP in both our cross validation and test calculation of $0.071$.
...
#### Results
Using cross validation, we obtain a $\lambda = 0.01$ as the optimal tuning parameter and a MSEP of $0.09$. The true test MSEP was extremely close at $0.1$.
```{r echo=FALSE}
library(glmnet)
...
#### Results
Using cross validation, we obtain a $\lambda = 0.01$ as the optimal tuning parameter and a MSEP of $0.710$. With the test set, we obtained a MSEP of $0.074$.
```{r echo=FALSE}
library(glmnet)
...
#### Results 
Using cross validation, we obtain a $\lambda = 0.01$ as the optimal tuning parameter and a MSEP of $0.07$. With the test set, we obtain an identical MSEP of $0.70$. 
```{r echo=FALSE}
library(glmnet)
...
cv_error <- c(0.0613,0.0613,0.0613,
              0.071,0.09,0.071,0.07,
              0.0845)

test_error <- c(0.105,0.105,0.105,
                0.071,0.1,0.074,0.07,
                0.094)

error_summary <- dplyr::tibble(method,cv_error,test_error)
