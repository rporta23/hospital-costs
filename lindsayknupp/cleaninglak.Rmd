...
# error = mean(test$costs_bin != knn_pred)
model_info
```
...
formula = c("costs_bin ~ .")
error_rates <- map_dbl(1:k, ~compute_MSEP_glm(train_data, formula, .x))

test_error_vec <- map_dbl( formulas, ~kfold_cv_glm(train, formula = .x, k = 10) )
test_error_vec
```
