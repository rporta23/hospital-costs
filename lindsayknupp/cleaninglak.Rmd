...
compute_MSEP_neural_network = function(train_data,nn,i){
  current_train_folds.x = filter(train_data, fold != i) %>% select(-c(total_costs,fold))
  current_train_folds.x = array_reshape(current_train_folds.x,
                                        c(nrow(current_train_folds.x),
                                          ncol(current_train_folds.x)))
  current_train_folds.y = filter(train_data, fold != i) %>% 
    select(total_costs)
   current_train_folds.y = array_reshape(current_train_folds.y,
                                         c(nrow(current_train_folds.y),1))

  current_test_fold.x = filter(train_data, fold == i) %>% select(-c(total_costs,fold))
  current_test_fold.x = array_reshape(current_test_fold.x,
                                               c(nrow(current_test_fold.x),
                                                 ncol(current_test_fold.x)))
  current_test_fold.y = filter(train_data, fold == i) %>% 
    select(total_costs)
 current_test_fold.y = array_reshape(current_test_fold.y,
                                         c(nrow(current_test_fold.y),1))
  ...
  error = history[["metrics"]][["val_loss"]][10]
  return(error)
} 
...
train_y = train %>%
  select(total_costs) 
train_y = array_reshape(train_y,c(nrow(train_y),ncol(train_y)))
...
test_y = test %>%
  select(total_costs) 
test_y = array_reshape(test_y,c(nrow(test_y),ncol(test_y)))
...
testMSE_neural_net = history[["metrics"]][["val_loss"]][10]
...
# # 
# knn_csv = write_csv(data.frame(error = knn_error_rates), file = "knn_error_rates.csv")

# optimal_n <- which.min(knn_error_rates)
# optimal_error_rate <- knn_error_rates[optimal_n]

optimal_n <- 7
optimal_error_rate <- 0.05329245
...
z_value = a[,3]
Pr = a[,4]
Coefficients = row.names(a)

summary_df = tibble("Coefficients" = Coefficients,"Estimate" = Estimate,
"Std. Error" = Std_error, "z value" = z_value, "Pr(>|Z|)" = Pr)

write_csv(summary_df,file = "simpleglm_coef.csv")
...
train = df_train %>%
  select(-c(total_costs)) %>%
  mutate(income_emp = total_income * fte_employees_on_payroll,
         sal_emp = salaries * fte_employees_on_payroll,
         total_days_sq = total_days^2,
         ) %>%
  select(total_income,fte_employees_on_payroll,salaries,total_days,
         total_days_sq, income_emp,sal_emp,costs_bin,fold)
...
test = df_test %>%
  ...
  select(total_income,fte_employees_on_payroll,salaries,total_days,
         total_days_sq, income_emp,sal_emp,costs_bin)

transformation_mod = glm("costs_bin ~ .", data = select(train,-fold),family = binomial)

a = summary(transformation_mod)[["coefficients"]]
Estimate = a[,1]
Std_error = a[,2]
z_value = a[,3]
Pr = a[,4]
...
write_csv(summary_df,file = "transglm_coef.csv")

pred <- round(predict(transformation_mod, newdata = select( test, -c(costs_bin) ),type = "response" ))
...
testMSE_trans_glm = model_info$error_rate
truepos_trans_glm = model_info$truepos
trueneg_trans_glm = model_info$trueneg
falsepos_trans_glm = model_info$falsepos
falseneg_trans_glm = model_info$falseneg


formula = c("costs_bin ~ .")
error_rates <- map_dbl(1:10, ~compute_MSEP_glm(train, formula, .x))
cv_trans_glm = mean(error_rates)
...
model_info = tibble(.rows=1)
contingency = table(lda_pred,test$costs_bin)
...
model_info = tibble(.rows=1)
contingency = table(qda_pred,test$costs_bin)
...
model_info$error_rate <- mean( (test$costs_bin != qda_pred) )
...
model_info = tibble(.rows=1)
contingency = table(nb_pred,test$costs_bin)
...
model_info$error_rate <- mean( (test$costs_bin != nb_pred) )
...
```
Best number of terminal nodes is 5.
...
compute_MSEP_regression_tree <- function(train_data,i){
    ...
    prune.costs = prune.tree(tree.costs,best = 5)
...
importance(random_forest.costs)
...
```{r, include = TRUE}
table <- summary.gbm(boost.costs, plotit = FALSE) |> 
  rename("Variables" = var,`Relative Influence` = rel.inf) 

write_csv(table,file="boosting.csv")
...
testMSE_neural_net = 0.0385289
```

## Qualitative summary
```{r}

method <- c("KNN","Multiple Logistic Regression","Multiple Logistic Regression (Transformations)","LDA","QDA","Naive Bayes (Gaussian)",
            "Naive Bayes (KDE)","Decision Tree","Bagging","Random Forest",
            "Boosting","Neural Net")

cv_error <- c(cv_knn,cv_glm,cv_trans_glm,cv_lda,cv_qda,cv_nb,cv_nb_kde,
              cv_decision_tree,
              cv_bagging,cv_random_forest,cv_boosting,cv_neural_network)

error_rate <- c(testMSE_knn,testMSE_glm,testMSE_trans_glm,testMSE_lda,
                testMSE_qda,testMSE_nb,
                testMSE_nb_kde,testMSE_decision_tree,testMSE_bagging,
                testMSE_random_forest,testMSE_boosting,testMSE_neural_net)

true_pos <- c(truepos_knn,truepos_glm,truepos_trans_glm,truepos_lda,
              truepos_qda,truepos_nb,
              truepos_nb_kde,truepos_decision_tree,truepos_bagging,
              truepos_random_forest,truepos_boosting,truepos_neural_net)

true_neg <- c(trueneg_knn,trueneg_glm,trueneg_trans_glm,trueneg_lda,
              trueneg_qda,trueneg_nb,
              trueneg_nb_kde,trueneg_decision_tree,trueneg_bagging,
              trueneg_random_forest,trueneg_boosting,trueneg_neural_net)

false_pos <- c(falsepos_knn,falsepos_glm,falsepos_trans_glm,falsepos_lda,
               falsepos_qda,falsepos_nb,
              falsepos_nb_kde,falsepos_decision_tree,falsepos_bagging,
              falsepos_random_forest,falsepos_boosting,falsepos_neural_net)

false_neg <- c(falseneg_knn,falseneg_glm,falseneg_trans_glm,falseneg_lda,
               falseneg_qda,falseneg_nb,
              falseneg_nb_kde,falseneg_decision_tree,falseneg_bagging,
              falseneg_random_forest,falseneg_boosting,falseneg_neural_net)

error_summary <- tibble(method,cv_error,error_rate,true_pos,true_neg,
                        false_pos,false_neg)
...
write_csv(error_summary,file="qualsummary.csv")
...
mod_mlr <- lm(formula= formula, data = train_data)
...
compute_min_k <- function(df_train, seed) {
  ...
  cat_df <- scaled_hpc_dummies |> select(c(provider_bin_Specialized, control_bin_Governmental, control_bin_Proprietary, rural, duplicate))
...
write_csv(results,file="sim_study_results.csv")
